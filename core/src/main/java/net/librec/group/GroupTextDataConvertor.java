...
/**
 * @author Joaqui TODO The data convertor for the group is the one that will
 *         have to generate the groups because it is the only one that has
 *         access to the data frame and can retrieve from it the sparse rating
 *         matrix
 *
 */
public class GroupTextDataConvertor extends AbstractDataConvertor {
	 ...
	 * threshold, the value will be 1; otherwise 0. To disable this appender, i.e.,
	 * keep the original rating value, set the threshold a negative value
	 */
		...
		for (File file : files) {
			try (Source fileSource = Okio.source(file); BufferedSource bufferedSource = Okio.buffer(fileSource)) {
				...
				while ((temp = bufferedSource.readUtf8Line()) != null) {
					...
					// TODO Test that here is where I need to change the row for adding the group
					// field
